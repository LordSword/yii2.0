...
/* 以读取为例，如果访问 $Component->property1   yii\base\Component::__get()
 * __get() 调用 ensureBehaviors()  遍历 behaviors() 同时 调用 attachBehaviorInternal()
 * attachBehaviorInternal() 先判断 behaviors()属于哪一类 再 调用 attach()
 * attach()  赋值 $owner 绑定事件
 *
 */
     ...
     * public function behaviors() {
                    return [
                        'access' => [
                                ...
                                'except' => ['reg-get-code', 'captcha', 'yirute-tid'],
                                'rules' => [
                                        [
                                            'allow' => true,
                                            'roles' => ['@'],
